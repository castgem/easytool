# 🌍 Translation Guide for ToolEasy

This guide will help you add new languages or improve existing translations for ToolEasy.
...
ToolEasy uses **i18next** for internationalization (i18n). Currently supported languages:
...
ToolEasy uses a **static pre-rendering** approach for SEO-optimized i18n:
...
6. Run `pnpm run build` to generate static language pages
7. Test thoroughly
...
ToolEasy is **friendly, clear, and professional**. Match this tone in your translations.
...
```json
{
  "faq.analytics.answer": "No. ToolEasy does not track you at all. We do not use cookies, analytics, or any tracking scripts, not even on the live website at usetooleasy.com. We never know who you are or what you do with the tools."
}
```
...
```json
{
  "faq.analytics.answer": "Nein. ToolEasy verfolgt Sie in keiner Weise. Wir verwenden keine Cookies, keine Analysetools und keine Tracking-Skripte – auch nicht auf der Live-Website usetooleasy.com. Wir wissen nie, wer Sie sind oder was Sie mit den Tools tun."
}
```
...
- [ ] Run `pnpm run build` and verify `dist/{lang}/` folder is created
...
2. Contact us via [contact.html](https://usetooleasy.com/contact.html) on the hosted site
...
2. **Send your locale files** (`public/locales/{lang}/common.json` and `tools.json`) via the contact form or your usual support channel
3. Include screenshots showing the translation in action and the language code you added

We review submissions for quality and merge approved translations in product releases.

Thank you for helping improve ToolEasy! 🎉
