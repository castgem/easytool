...
# ============================================================
# ToolEasy Air-Gapped Deployment Preparation Script
# ============================================================
...
# contains everything needed to deploy ToolEasy offline.
#
# Usage:
...
WASM_BASE_URL=""
IMAGE_NAME="tooleasy"
OUTPUT_DIR="./tooleasy-airgap-bundle"
SIMPLE_MODE=""
BASE_URL=""
COMPRESSION_MODE=""
LANGUAGE=""
...
usage() {
  cat <<'EOF'
ToolEasy Air-Gapped Deployment Preparation
...
OPTIONS:
  --image-name <name>     Docker image name          (default: tooleasy)
  --output-dir <path>     Output bundle directory     (default: ./tooleasy-airgap-bundle)
...
if [ ! -f "package.json" ] || [ ! -f "src/js/const/cdn-version.ts" ]; then
  error "This script must be run from the ToolEasy project root."
...
read_versions() {
  ...
  TESSERACT_VERSION=$(node -p "require('tesseract.js/package.json').version")
  TESSERACT_CORE_VERSION=$(node -p "require('tesseract.js-core/package.json').version")
...
interactive_mode() {
  echo ""
  echo -e "${BOLD}============================================================${NC}"
  echo -e "${BOLD}  ToolEasy Air-Gapped Deployment Preparation${NC}"
  ...
  echo "      https://cdn.mycompany.local/tooleasy"
  echo ""
  while true; do
  ...
  echo "    Replace the default ToolEasy name, logo, and footer text."
  read -r -p "    Brand name [ToolEasy]: " input
  BRAND_NAME="${input:-}"
  if [ -n "$BRAND_NAME" ]; then
    ...
    BRAND_LOGO="${input:-}"
    read -r -p "    Footer text [© 2026 ToolEasy. All rights reserved.]: " input
    FOOTER_TEXT="${input:-}"
  fi
  echo ""
  ...
  echo "  Brand Name:     ${BRAND_NAME:-ToolEasy (default)}"
...
echo ""
echo -e "${BOLD}============================================================${NC}"
echo -e "${BOLD}  ToolEasy Air-Gapped Bundle Preparation${NC}"
...
if ls "$OUTPUT_DIR"/*.tgz "$OUTPUT_DIR"/tooleasy.tar "$OUTPUT_DIR"/setup.sh 2>/dev/null | head -1 &>/dev/null; then
...
if [ "$SKIP_WASM" = true ]; then
  ...
  wasm_missing=false
  if ! ls "$OUTPUT_DIR"/tooleasy-pymupdf-wasm-*.tgz &>/dev/null; then
    error "Missing: tooleasy-pymupdf-wasm-*.tgz"
    wasm_missing=true
  fi
  if ! ls "$OUTPUT_DIR"/tooleasy-gs-wasm-*.tgz &>/dev/null; then
    error "Missing: tooleasy-gs-wasm-*.tgz"
    wasm_missing=true
  fi
  ...
  info "Downloading @tooleasy/pymupdf-wasm@${PYMUPDF_VERSION}..."
  if ! (cd "$WASM_TMP" && npm pack "@tooleasy/pymupdf-wasm@${PYMUPDF_VERSION}" --quiet 2>&1); then
    error "Failed to download @tooleasy/pymupdf-wasm@${PYMUPDF_VERSION}"
    error "Check your internet connection and that the package exists on npm."
    exit 1
  fi

  info "Downloading @tooleasy/gs-wasm@${GS_VERSION}..."
  if ! (cd "$WASM_TMP" && npm pack "@tooleasy/gs-wasm@${GS_VERSION}" --quiet 2>&1); then
    error "Failed to download @tooleasy/gs-wasm@${GS_VERSION}"
    error "Check your internet connection and that the package exists on npm."
    exit 1
  fi
  ...
  info "  PyMuPDF:      $(filesize "$OUTPUT_DIR"/tooleasy-pymupdf-wasm-*.tgz)"
  info "  Ghostscript:  $(filesize "$OUTPUT_DIR"/tooleasy-gs-wasm-*.tgz)"
...
if [ "$SKIP_DOCKER" = true ]; then
  ...
  if [ -f "$OUTPUT_DIR/tooleasy.tar" ]; then
    success "Reusing existing tooleasy.tar"
  elif docker image inspect "$IMAGE_NAME" &>/dev/null; then
    step "Exporting existing Docker image"
    docker save "$IMAGE_NAME" -o "$OUTPUT_DIR/tooleasy.tar"
    success "Exported: $(filesize "$OUTPUT_DIR/tooleasy.tar")"
  else
    warn "No Docker image '${IMAGE_NAME}' found and no tooleasy.tar in output."
  ...
  echo ""
  info "Building... this may take a few minutes (pnpm install + Vite build)."
  echo ""
...
# ============================================================
# ToolEasy Air-Gapped Setup Script
# Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
# ToolEasy v${APP_VERSION}
# ============================================================
...
echo ""
echo "============================================================"
echo "  ToolEasy Air-Gapped Setup"
...
if [ -f "\${SCRIPT_DIR}/tooleasy.tar" ]; then
  docker load -i "\${SCRIPT_DIR}/tooleasy.tar"
  echo "  Docker image '\${IMAGE_NAME}' loaded."
else
  echo "  WARNING: tooleasy.tar not found. Skipping Docker load."
...
tar xzf "\${SCRIPT_DIR}"/tooleasy-pymupdf-wasm-*.tgz -C "\${WASM_DIR}/pymupdf" --strip-components=1

# Ghostscript: browser expects gs.js and gs.wasm at root
echo "  Extracting Ghostscript..."
TEMP_GS="\$(mktemp -d)"
tar xzf "\${SCRIPT_DIR}"/tooleasy-gs-wasm-*.tgz -C "\${TEMP_GS}"
...
# --- Step 3: Start ToolEasy ---
echo ""
echo "[3/3] Ready to start ToolEasy"
echo ""
echo "  To start manually:"
echo "    docker run -d --name tooleasy -p \${DOCKER_PORT}:8080 --restart unless-stopped \${IMAGE_NAME}"
echo ""
echo "  Then open: http://localhost:\${DOCKER_PORT}"
echo ""

read -r -p "Start ToolEasy now? (y/N): " REPLY
if [[ "\${REPLY:-}" =~ ^[Yy]$ ]]; then
  docker run -d --name tooleasy -p "\${DOCKER_PORT}:8080" --restart unless-stopped "\${IMAGE_NAME}"
  echo ""
  echo "  ToolEasy is running at http://localhost:\${DOCKER_PORT}"
fi
SETUP_EOF
...
# ToolEasy Air-Gapped Deployment Bundle

**ToolEasy v${APP_VERSION}** | Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")

## Contents

| File | Description |
| --- | --- |
| \`tooleasy.tar\` | Docker image |
| \`tooleasy-pymupdf-wasm-${PYMUPDF_VERSION}.tgz\` | PyMuPDF WASM module |
| \`tooleasy-gs-wasm-${GS_VERSION}.tgz\` | Ghostscript WASM module |
...
3. Optionally start the ToolEasy container
...
# PyMuPDF
tar xzf tooleasy-pymupdf-wasm-${PYMUPDF_VERSION}.tgz -C ./wasm/pymupdf --strip-components=1

# Ghostscript (extract assets/ to root)
TEMP_GS=\$(mktemp -d)
tar xzf tooleasy-gs-wasm-${GS_VERSION}.tgz -C \$TEMP_GS
...
\`\`\`bash
docker run -d --name tooleasy -p 3000:8080 --restart unless-stopped ${IMAGE_NAME}
\`\`\`
