...
const CACHE_VERSION = 'tooleasy-v11';
      ...
      .then((cacheNames) => {
        return Promise.all(
          cacheNames.map((cacheName) => {
            if (cacheName.startsWith('tooleasy-') && cacheName !== CACHE_NAME) {
...
function shouldCache(pathname, isCDN = false) {
  if (isCDN) {
    return (
      pathname.includes('/@tooleasy/pymupdf-wasm') ||
      pathname.includes('/@tooleasy/gs-wasm') ||
