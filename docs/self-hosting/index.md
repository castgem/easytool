...
ToolEasy can be self-hosted on your own infrastructure. This guide covers various deployment options.
...
> [!IMPORTANT]
> Office tools need COOP/COEP headers and HTTPS on LAN IPs. The Docker image sets the headers; use TLS for `http://192.168.x.x` access.
...
Create `~/.config/containers/systemd/tooleasy.container`:
...
Build first: `docker build -t tooleasy:latest .`

```bash
systemctl --user daemon-reload
systemctl --user enable --now tooleasy
```
...
`SIMPLE_MODE=true` hides the marketing homepage. Same image name `tooleasy:latest`:
...
docker run -p 8080:8080 tooleasy:latest
```

Deploy to a subdirectory:

```bash
BASE_URL=/pdf-tools/ pnpm run build
```

### Custom Branding

Replace the default ToolEasy logo, name, and footer text with your own at build time:

| Variable           | Description                           | Default                                 |
| ------------------ | ------------------------------------- | --------------------------------------- |
| `VITE_BRAND_NAME`  | Brand name shown in header and footer | `ToolEasy`                              |
...
```bash
DISABLE_TOOLS="edit-pdf,sign-pdf" pnpm run build
```
...
**Verify**: open DevTools Console on any ToolEasy page and run:
...
## WASM Configuration (optional modules)

ToolEasy **does not bundle** heavy processing libraries in its source tree, but **pre-configures CDN URLs** so advanced features work out of the box — no manual setup needed.
...
| Component       | Notes    | Features                                                         |
| --------------- | -------- | ---------------------------------------------------------------- |
| **PyMuPDF**     | Optional | EPUB/MOBI/FB2/XPS conversion, image extraction, table extraction |
| **Ghostscript** | Optional | PDF/A conversion, compression, deskewing, rasterization          |
| **CoherentPDF** | Optional | Table of contents, attachments, PDF merge with bookmarks         |
...
```bash
VITE_WASM_PYMUPDF_URL=https://cdn.jsdelivr.net/npm/@tooleasy/pymupdf-wasm@0.11.16/
VITE_WASM_GS_URL=https://cdn.jsdelivr.net/npm/@tooleasy/gs-wasm@0.1.1/assets/
...
VITE_WASM_PYMUPDF_URL=https://your-server.com/pymupdf/ pnpm run build
```
...
| `--image-name <name>`          | Docker image tag                                 | `tooleasy`                        |
| `--output-dir <path>`          | Output bundle directory                          | `./tooleasy-airgap-bundle`        |
...
WASM files must be served from the **same origin** as the ToolEasy app. Web Workers use `importScripts()` which cannot load scripts cross-origin. For example, if ToolEasy runs at `https://internal.example.com`, the WASM base URL should also be `https://internal.example.com/wasm`.
:::
...
tar xzf tooleasy-pymupdf-wasm-0.11.14.tgz -C ./wasm/pymupdf --strip-components=1
tar xzf tooleasy-gs-wasm-*.tgz -C ./wasm/gs --strip-components=1
...
Set the variables in `.env.production` before running `pnpm run build`:
