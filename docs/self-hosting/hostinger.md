...
[Hostinger](https://hostinger.com) is a popular shared hosting provider. This guide covers deploying ToolEasy to Hostinger's shared hosting.
...
RewriteCond %{HTTP_HOST} ^usetooleasy\.com [NC]
RewriteRule ^(.*)$ https://www.usetooleasy.com/$1 [L,R=301]
...
Make sure the `.html` extension rule and language routes are correctly configured. ToolEasy uses static HTML files, not SPA routing:
