...
The easiest way to self-host ToolEasy in a production environment.
...
Image: **`tooleasy:latest`** (built locally). To hide marketing UI: `docker build --build-arg SIMPLE_MODE=true -t tooleasy:latest .`

```bash
docker run -d --name tooleasy -p 8080:8080 --restart unless-stopped tooleasy:latest
```
...
```bash
docker build -t tooleasy:custom .
docker run -d -p 3000:8080 tooleasy:custom
```
...
- Air-gapped deployments that override all three `VITE_WASM_*_URL` values also get the public `cdn.jsdelivr.net` removed from CSP (each default is replaced, not appended). Similarly, setting `VITE_CORS_PROXY_URL` replaces the public `tooleasy-cors-proxy.usetooleasy.workers.dev` default.
...
docker build --build-arg VITE_DEFAULT_LANGUAGE=fr -t tooleasy .
docker run -d -p 3000:8080 tooleasy
```

### Custom Branding

Replace the default ToolEasy logo, name, and footer text with your own. Place your logo file in the `public/` folder (or use an existing image), then pass the branding variables at build time:
...
Tool IDs are the page URL without `.html`. For example, if the tool lives at `usetooleasy.com/edit-pdf.html`, the ID is `edit-pdf`.

#### Finding Tool IDs

The easiest way: open any tool in ToolEasy and look at the URL. The last part of the path (without `.html`) is the tool ID.
...
npm pack @tooleasy/pymupdf-wasm@0.11.14
npm pack @tooleasy/gs-wasm
...
# 4. Export the image
docker save tooleasy -o tooleasy.tar

# 5. Transfer tooleasy.tar + the .tgz packages + tesseract-langdata/ + ocr-fonts/ into the air-gapped network
...
tar xzf tooleasy-pymupdf-wasm-0.11.14.tgz -C /var/www/wasm/pymupdf --strip-components=1
tar xzf tooleasy-gs-wasm-*.tgz -C /var/www/wasm/gs --strip-components=1
...
# Run ToolEasy
docker run -d -p 3000:8080 --restart unless-stopped tooleasy
```
...
By default, ToolEasy listens on port `8080` inside the container. To change this, set the `PORT` environment variable at runtime:

```bash
docker run -p 3000:9090 -e PORT=9090 tooleasy:latest
```
...
For environments that require running as a specific non-root user (NAS devices, Kubernetes with security contexts, organizational policies), ToolEasy provides a separate Dockerfile with LSIO-style PUID/PGID support.

### Build and Run

```bash
# Build the non-root image
docker build -f Dockerfile.nonroot -t tooleasy-nonroot .
...
services:
  ...
  tooleasy:
    ...
    labels:
      - 'traefik.enable=true'
      - 'traefik.http.routers.tooleasy.rule=Host(`pdf.example.com`)'
      - 'traefik.http.routers.tooleasy.entrypoints=websecure'
      - 'traefik.http.routers.tooleasy.tls.certresolver=letsencrypt'
      - 'traefik.http.services.tooleasy.loadbalancer.server.port=8080'
      # Required headers for SharedArrayBuffer (LibreOffice WASM)
      - 'traefik.http.routers.tooleasy.middlewares=tooleasy-headers'
      - 'traefik.http.middlewares.tooleasy-headers.headers.customresponseheaders.Cross-Origin-Opener-Policy=same-origin'
      - 'traefik.http.middlewares.tooleasy-headers.headers.customresponseheaders.Cross-Origin-Embedder-Policy=require-corp'
    restart: unless-stopped
```
...
Create a container unit file at `~/.config/containers/systemd/tooleasy.container` (user) or `/etc/containers/systemd/tooleasy.container` (system):

```ini
[Unit]
Description=ToolEasy - Privacy-first PDF toolkit
After=network-online.target
Wants=network-online.target

[Container]
Image=tooleasy:latest
ContainerName=tooleasy
...
# Start the service
systemctl --user start tooleasy

# Enable on boot
systemctl --user enable tooleasy

# Check status
systemctl --user status tooleasy

# View logs
journalctl --user -u tooleasy -f
```
...
```ini
[Unit]
Description=ToolEasy with health monitoring
After=network-online.target
Wants=network-online.target
...
For custom network configuration, create a network file `tooleasy.network`:
