...
Additionally, many CA servers serve certificates over plain HTTP. When your ToolEasy instance is hosted over HTTPS, browsers block these HTTP requests (mixed content policy). The CORS proxy resolves both issues by routing requests through an HTTPS endpoint with proper headers.
...
If you're self-hosting ToolEasy, you'll need to deploy your own CORS proxy for digital signatures to work with certificates that require chain fetching.
...
3. **Clone ToolEasy and update allowed origins**:
   ...
   ::: warning Important
   Without this change, the proxy will reject all requests from your site with a **403 Forbidden** error. The default only allows requests from `usetooleasy.com`.
   :::
...
4. **Deploy the proxy**:
   ...
   Note your worker URL (e.g., `https://tooleasy-cors-proxy.your-subdomain.workers.dev`).

5. **Rebuild ToolEasy with the proxy URL**:
   ...
   ```bash
   VITE_CORS_PROXY_URL=https://your-worker.workers.dev pnpm run build
   ```
