...
Host ToolEasy on your own server using Nginx.
...
Create `/etc/nginx/sites-available/tooleasy`:
...
```bash
sudo ln -s /etc/nginx/sites-available/tooleasy /etc/nginx/sites-enabled/
sudo nginx -t
