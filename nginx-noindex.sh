...
MARKER='<!-- tooleasy-noindex-injected -->'
...
find "$ROOT" -type f -name '*.html' | while read -r file; do
    if [ ! -w "$file" ]; then
        entrypoint_log "Skipping $file (not writable)"
        continue
    fi
    ...
    if grep -q '<meta name="robots" content="noindex' "$file"; then
        sed -i "1a\\
$MARKER
" "$file" || entrypoint_log "WARNING: failed to update $file"
        continue
    fi
    sed -i "s|</head>|    $META_TAG\\
    $MARKER\\
  </head>|" "$file" || entrypoint_log "WARNING: failed to update $file"
done
