...
function engineVersion(): string {
  try {
    const dir = resolve(__dirname, 'node_modules/tooleasy-pdfium');
...
function buildCorsProxyAllowedHosts(): Set<string> {
  const hosts = new Set<string>([
    ...
    'fonts.gstatic.com',
    'tooleasy-cors-proxy.usetooleasy.workers.dev',
    ...
    req.on('end', () => {
      ...
      const proxyReq =
        target.protocol === 'https:'
          ? https.request(targetUrl, requestOptions, handleProxyResponse)
          : http.request(targetUrl, requestOptions, handleProxyResponse);
...
export default defineConfig(() => {
  ...
  return {
    ...
    optimizeDeps: {
      ...
      exclude: ['coherentpdf', 'wasm-vips', 'tooleasy-pdfium'],
    },
    server: {
      host: process.env.VITE_DEV_HOST || 'localhost',
      watch: {
        ignored: ['!**/node_modules/tooleasy-pdfium/**'],
      },
      headers: {
    ...
    build: {
      rollupOptions: {
        input: {
          ...
          'base64-tool': resolve(__dirname, 'src/pages/base64-tool.html'),
          'json-formatter': resolve(__dirname, 'src/pages/json-formatter.html'),
          'hash-generator': resolve(
            __dirname,
            'src/pages/hash-generator.html'
          ),
        },
        output: {
