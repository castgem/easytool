...
echo "Starting ToolEasy with PUID=$PUID PGID=$PGID"

addgroup -g "$PGID" tooleasy 2>/dev/null || true
adduser -u "$PUID" -G tooleasy -D -H -s /sbin/nologin tooleasy 2>/dev/null || true
...
if [ "${ROBOTS_NOINDEX:-false}" = "true" ] && [ -x /usr/local/bin/tooleasy-noindex.sh ]; then
    NOINDEX_HTML_ROOT=/usr/share/nginx/html /usr/local/bin/tooleasy-noindex.sh || true
fi
